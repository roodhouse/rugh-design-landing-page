...
const Record = (props) => (
  <tr className="border-b bg-[gainsboro] text-[#676766]">
    ...
    <Link className="text-blue-700 flex items-center" to={`/review/${props.record.slug}`}><span className="inline-block mr-1"><FaEye /></span>View</Link> |
...
export default function RecordList() {
  ...
  useEffect(() => {
    async function getRecords() {
      const response = await fetch(`http://localhost:5001/record/` || `https://rugh.design:5001/record/` || `${process.env.URL}:/5001/record`);
  ...
  async function deleteRecord(id) {
    await fetch(`http://localhost:5001/${id}` || `https://rugh.design:5001/${id}` || `${process.env.URL}:/5001/${id}`, {
      method: "DELETE"
    });
