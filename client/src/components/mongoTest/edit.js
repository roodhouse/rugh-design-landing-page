...
const modules = {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(`http://localhost:5001/record/${params.id.toString()}` || `https/rugh.design:5001/record/${params.id.toString()}` || `${process.env.URL}:/5001/record/${params.id.toString()}`);
  ...
  async function onSubmit(e) {
    ...
    await fetch(`http://localhost:5001/update/${params.id}` || `https://rugh.design:5001/update/${params.id}` || `${process.env.URL}:/5001/update/${params.id}`, {
      method: "POST",
